...
void setup()
{
    ...
    attachInterrupt(digitalPinToInterrupt(RX0), &rising0, RISING);//left and right throttle (main out 3 on pixhawk)
    attachInterrupt(digitalPinToInterrupt(RX1), &rising1, RISING);//left and right steering (main out 1 on pixhawk)
    attachInterrupt(digitalPinToInterrupt(RX2), &rising2, RISING);//emergency stop (pwm 6 on Pixhawk)
